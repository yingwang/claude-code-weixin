 ...
 */
export async function loginFlow(apiBaseUrl = "https://ilinkai.weixin.qq.com") {
    const api = new WeixinLoginApi(apiBaseUrl);
    console.log("\n正在获取登录二维码...\n");
    const qrRes = await api.getQrCode();
    if (qrRes.ret !== 0 || !qrRes.qrcode) {
        throw new Error(`获取二维码失败: ret=${qrRes.ret} ${qrRes.err_msg || ""}`);
    }
    ...
    qrcode.generate(qrcodeUrl, { small: true }, (code) => {
        console.log(code);
    });
    console.log(`\n请使用微信扫描上方二维码登录\n`);
