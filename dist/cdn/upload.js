 ...
 */
export async function uploadFile(api, filePath, toUserId, mediaType = 3 // 1=IMAGE, 2=VIDEO, 3=FILE
) {
    ...
    logger.debug(`Uploading ${fileName} (${rawData.length} bytes, encrypted: ${encrypted.length} bytes)`);
    ...
    if (!uploadRes.upload_param) {
        throw new Error(`getUploadUrl failed: ret=${uploadRes.ret} ${uploadRes.errmsg || uploadRes.err_msg || ""}`);
    }
